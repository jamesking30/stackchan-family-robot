...
| 合成 | GPT-SoVITS v2 / Elysia e20 + e24 | 使用本地训练权重和开心参考音频，音色更接近目标角色 |
| 合成备用 | Qwen3-TTS 0.6B CustomVoice 8-bit / Vivian | 主音色不可用时自动接管中英文语音 |
...
ROBOT_VOICE_GPT_SOVITS_REF_AUDIO=var/models/gpt-sovits/elysia/reference-happy.wav
ROBOT_VOICE_GPT_SOVITS_PROMPT_TEXT=所以你今天就来见我了吗？哇，真令人开心呢。
ROBOT_VOICE_GPT_SOVITS_PROMPT_LANG=zh
ROBOT_VOICE_GPT_SOVITS_SPEED=0.96
ROBOT_VOICE_TTS_BASE_URL=http://127.0.0.1:8766
ROBOT_VOICE_TTS_MODEL=mlx-community/Qwen3-TTS-12Hz-0.6B-CustomVoice-8bit
ROBOT_VOICE_TTS_SPEAKER=Vivian
ROBOT_VOICE_TTS_INSTRUCTION=明亮、轻快、活泼、有精神，语速自然舒展，停顿从容；保持亲切自然，不要慵懒拖音，不要急促，不要尖叫或夸张卖萌。
ROBOT_VOICE_TTS_SPEED=0.96
...
```bash
./scripts/setup_gpt_sovits.sh "/path/to/【GPT-SoVITS 2.0】爱莉希雅"
./scripts/setup_local_tts.sh
./scripts/install_launch_agent.sh
```

服务启动后自动预热 GPT-SoVITS 的模型与参考音频缓存。主音色不可用时依次回退到 Qwen 和 macOS `say`，但不会调用云端 TTS。长期运行和固定局域网地址的操作见 [`operations-reliability.md`](operations-reliability.md)。
