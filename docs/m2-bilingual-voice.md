...
5. Mac 上的 Qwen3-TTS 神经语音在本地生成 24kHz PCM，再编码成 60ms Opus 帧下发；回答文字同时显示在脸部界面。
...
| 合成 | Qwen3-TTS 0.6B CustomVoice 8-bit / Serena | M5 上本地推理，支持中英文和语调控制 |
...
ROBOT_VOICE_TTS_SPEAKER=Serena
ROBOT_VOICE_TTS_INSTRUCTION=温暖、自然、亲切，像熟悉的家人一样说话。
...
TTS 使用独立虚拟环境和本机端口 `8766`，不与机器人主服务混装依赖。首次执行会下载约 2GB 模型，之后直接使用 Mac 缓存。

```bash
./scripts/setup_local_tts.sh
./scripts/run_robot_services.sh
```

如本地神经语音服务不可用，主服务会回退到 macOS `say`，但不会调用云端 TTS。
